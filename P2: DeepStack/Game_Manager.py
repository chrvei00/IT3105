...
class Game:
    def __init__(self, Num_Human_Players: int, Num_AI_Players: int, Game_Type: str = "simple", start_chips: int = 1000):
        util.validate_game(Num_Human_Players, Num_AI_Players, Game_Type)
        self.players, self.dealer, self.deck, self.blind = util.setup_game(Num_Human_Players, Num_AI_Players, Game_Type, start_chips)
    ...
    def start(self):
        while not util.game_over(self.players):
            ...
            # Play a hand
            hand = Hand(self.players, self.dealer, self.deck, self.blind)
            print(hand)
            hand.play()
