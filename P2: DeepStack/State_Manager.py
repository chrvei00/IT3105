...
def generate_root(state: Node.State) -> Node.Node:
    ...
    """
    return Node.Node(state=state, depth=0, regret_sum=util.gen_hole_pair_matrix(), strategy_sum=util.gen_hole_pair_matrix())
    ...
    if depth < end_depth and node.children == []:
        if node.state.type == "decision":
            ...
            for action in util.possible_actions(node):
                node.add_child( Node.Node(generate_child_state(state=node.state, object=action), regret_sum=util.gen_hole_pair_matrix(), strategy_sum=util.gen_hole_pair_matrix()) )
            ...
            for card in util.possible_cards(node.state, max=rollouts):
                node.add_child( Node.Node(generate_child_state(state=node.state, object=card), regret_sum=util.gen_hole_pair_matrix(), strategy_sum=util.gen_hole_pair_matrix()))
